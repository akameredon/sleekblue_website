...
export default function BlogPage() {
  ...
  function changeCategory(cat) { setActiveCategory(cat); setPage(1) }
      ...
      <div style={{ maxWidth: '960px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '28px', marginTop: '16px' }}>
            {categories.map(cat => (
              <button key={cat} onClick={() => changeCategory(cat)}
                style={{ padding: '6px 16px', borderRadius: '20px', border: 'none', cursor: 'pointer', background: activeCategory === cat ? PRI : '#fff', color: activeCategory === cat ? '#fff' : '#555', fontWeight: activeCategory === cat ? 700 : 500, fontSize: '13px', fontFamily: "'HubotSans', sans-serif", boxShadow: '0 1px 4px rgba(0,0,0,0.08)', transition: 'all 0.15s' }}>
                {cat}
              </button>
            ))}
          </div>
        ...
        {!loading && filtered.length === 0 && (
          <div style={{ textAlign: 'center', padding: '60px 24px', background: '#fff', borderRadius: '16px', boxShadow: '0 1px 6px rgba(0,0,0,0.06)' }}>
            <div style={{ fontSize: '40px', marginBottom: '16px' }}>{search ? '🔍' : '✍️'}</div>
            ...
            <p style={{ fontSize: '14px', color: '#888', fontFamily: "'HubotSans', sans-serif" }}>
              {search ? 'Try a different search term or browse all categories.' : 'Check back soon for tips, guides, and insights.'}
            </p>
            {search && (
            ...
            >
              {post.coverImage && (
                <div style={{ width: '240px', flexShrink: 0 }}>
                  <img src={post.coverImage} alt={post.title} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', minHeight: '160px', maxHeight: '220px' }} loading="lazy" />
                </div>
              )}
              <div style={{ padding: '24px', flex: 1 }}>
                ...
                {post.excerpt && (
                  <p style={{ fontSize: '14px', color: '#666', lineHeight: 1.65, margin: '0 0 14px', fontFamily: "'HubotSans', sans-serif" }}>{post.excerpt}</p>
                )}
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  {post.audioUrl && <span style={{ fontSize: '12px' }}>🎙️</span>}
                </div>
              </div>
            </article>
          ))}
